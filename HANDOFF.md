...
- ⬜ 실제 교회 정보 반영 (아래 §6 placeholder 목록)
...
└── site/               ← Astro 5 프로젝트 (실제 사이트)
    ...
    ├── src/data/church.ts      ★ 교회 정보 단일 소스 (전부 placeholder)
...
## 6. Placeholder — 실제 정보로 교체 필요

`site/src/data/church.ts`에 모여 있음:
- 예배 시간표 전체 (현재 값은 전형적인 예시일 뿐 — 사이트에도 "시안용 예시" 표기가 있음. 실제 시간 확인 전까지 이 표기를 지우지 말 것)
- 주소("경기도 안양시 ○○구 ○○로 00"), 전화("031-000-0000"), 담임목사 성함("○○○")
- 이미지 4장은 AI 생성물 — 최종 오픈 전 실제 교회 사진으로 교체 권장 (특히 인물)
- "개척 40여 년"은 원 사이트의 "42년째(2022년 기준)" 기술에서 보수적으로 옮긴 것. 정확한 창립연도 확인 필요
- 지도 영역은 placeholder div — 카카오맵 연동 예정
...
3. **실제 정보 반영** (§6)
4. **XE DB 마이그레이션**: 사용자가 mysqldump + files/attach 백업 제공 예정. `xe_documents` 테이블 → Markdown 변환 스크립트 작성. 게시판: 생명의말씀(~440글)·공지(~360글)·갤러리·선교
5. **서브페이지 확장**: 교회소개, 선교, 공지, 온라인 주보(concept-e 재활용 가능)
6. **도메인 전환**: 기존 서버(nginx, self-signed cert 문제 있음)에서 새 호스팅으로. 사용자가 도메인 관리 권한 보유
